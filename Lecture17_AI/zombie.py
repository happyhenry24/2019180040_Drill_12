...
class Zombie:
    ...
    def is_boy_ball_count_more(self):
    ...
    def build_behavior_tree(self):
        ...
        a2 = Action('Move to random location', self.move_to)

        c1 = Condition('소년이 근처에 있는가?', self.is_boy_nearby, 7)
        c2 = Condition('소년의 공 개수가 더 많은가?', self.is_boy_ball_count_more)

        c2_negated = Condition('소년의 공 개수가 적은가?',
                               lambda: BehaviorTree.FAIL if self.is_boy_ball_count_more() == BehaviorTree.SUCCESS else BehaviorTree.SUCCESS)

        a3 = Action('소년에게 접근', self.move_to_boy)
        a4 = Action('소년에게서 도망가기', self.move_away_from_boy)

        chase_boy = Sequence('소년을 추적', c1, c2_negated, a3)
        flee_boy = Sequence('소년에게서 도망', c1, c2, a4)
        wander = Sequence('배회', a1, a2)

        root = Selector('추적 또는 배회', flee_boy, chase_boy, wander)
